...
interface VehiculoDao {
    ...
    suspend fun insertarVehiculo(vehiculo: Vehiculo): Long
    ...
    @Query("UPDATE vehiculo SET dynamoId = :dynamoId WHERE id = :id")
    suspend fun actualizarDynamoId(id: Int, dynamoId: Long)

    @Query("SELECT dynamoId FROM vehiculo WHERE id = :id")
    suspend fun obtenerDynamoId(id: Int): Long?

    @Query("UPDATE vehiculo SET dynamoId = NULL WHERE dynamoId IN (:ids)")
    suspend fun resetDynamoIds(ids: List<Long>)

    @Query("UPDATE vehiculo SET imagen = :url WHERE id = :id")
