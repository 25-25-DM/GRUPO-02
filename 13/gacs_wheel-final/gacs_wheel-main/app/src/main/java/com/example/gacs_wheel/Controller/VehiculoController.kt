...
object VehiculoController {
    ...
    suspend fun editarVehiculo(
        ...
        // 1. Verificar si el vehículo está inactivo
        if (!vehiculo.activo) {
            try {
                // 2. Obtener mensaje del servicio msgedit
                val mensaje = withContext(Dispatchers.IO) {
                    try {
                        val url = URL("https://admyb1ess2.execute-api.us-east-1.amazonaws.com/default/msgedit")
                        val connection = url.openConnection() as HttpURLConnection
                        connection.apply {
                            requestMethod = "GET"
                            connectTimeout = 3000  // 3 segundos de timeout
                            readTimeout = 3000
                            doInput = true
                        }
                        ...
                        if (connection.responseCode == 200) {
                            ...
                            JSONObject(response).getString("mensaje")
                        } else {
                            "MSGSEDIT_ERROR: Imposible editar vehiculo" // Mensaje por defecto si falla
                        }
                    } catch (e: Exception) {
                        Log.e("MSGSEDIT", "Error en servicio: ${e.message}")
                        "Imposible editar vehiculo" // Mensaje por defecto si hay error
                    }
                }

                // 3. Mostrar mensaje en UI (hilo principal)
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(context, mensaje, Toast.LENGTH_LONG).show()
                }
                return false // Abortar edición
            } catch (e: Exception) {
                Log.e("MSGSEDIT_ERROR", "Error general: ${e.message}", e)
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(context, "MSGSEDIT_ERROR: Imposible editar vehiculo", Toast.LENGTH_LONG).show()
                }
                return false
            }
        }

        // 4. Continuar con la edición si el vehículo está activo
        val dao = GascDataBase.getDatabase(context).vehiculoDao()
        val rows = dao.actualizarVehiculo(vehiculo)
        if (rows <= 0) return false

        // 5. Sincronización con AWS si hay credenciales
        if (awsAccessKey != null && awsSecretKey != null && awsSessionToken != null) {
            withContext(Dispatchers.IO) {
                try {
                    ...
                    if (dynamoId != null) {
                        ...
                        // Si la imagen no es URL, subir a S3
                        if (!imageUrl.startsWith("http")) {
                    ...
                    Log.e("SYNC_ERROR", "Error en sincronización AWS", e)
                }
            }
        } else {
            _refreshTrigger.value++
    ...
    // Función para obtener el mensaje del servicio
    private suspend fun obtenerMensajeMsgedit(): String = withContext(Dispatchers.IO) {
        val url = URL("https://admyb1ess2.execute-api.us-east-1.amazonaws.com/default/msgedit")
        val connection = url.openConnection() as HttpURLConnection
        ...
        return@withContext try {
            if (connection.responseCode == HttpURLConnection.HTTP_OK) {
                ...
                "MSGSEDIT_ERROR: Imposible editar vehiculo"
            }
        } finally {
    ...
    // Función para mostrar el mensaje en UI
